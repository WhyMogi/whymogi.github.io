...
We associate above with line bundle. If $C=V(f)\subset \bP^n$ is a curve of **degree** $d$ (meaning $f$ is a homogeneous polynomial of degree $d$ and we will define it in genral setting), then the ideal sheaf of $C$ is $O_X(d)$. Consider exact sequence \\[0\rightarrow k[x_0,\cdots,x_n](-d)\rightarrow\\]
